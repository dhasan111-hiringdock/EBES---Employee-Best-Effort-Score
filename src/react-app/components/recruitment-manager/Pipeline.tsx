  ...
  const saveRmReview = async (submissionId?: number) => {
    ...
    const res = await fetchWithAuth(`/api/rm/submissions/${submissionId}/review`, {
      ...
      body: JSON.stringify({
        ...
        rm_rate_bill: edits.rm_rate_bill ?? undefined,
        rm_rate_pay: edits.rm_rate_pay ?? undefined,
                            ...
                            <div className="md:col-span-9">
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-3">
                                ...
                                <input
                                  ...
                                  onChange={(e) => setRmEdits((prev) => ({ ...prev, [(row.submission_id || 0)]: { ...(prev[row.submission_id || 0] || {}), rm_rate_bill: Number(e.target.value) } }))}
                                  className="px-3 py-2 border border-gray-300 rounded-lg text-sm"
                                  disabled={!row.submission_id}
                                />
                                <input
                                  ...
                                  defaultValue={row.rm_rate_pay !== undefined ? Number(row.rm_rate_pay) : undefined}
                                  onChange={(e) => setRmEdits((prev) => ({ ...prev, [(row.submission_id || 0)]: { ...(prev[row.submission_id || 0] || {}), rm_rate_pay: Number(e.target.value) } }))}
                                  className="px-3 py-2 border border-gray-300 rounded-lg text-sm"
                                  disabled={!row.submission_id}
                                />
                                <input
